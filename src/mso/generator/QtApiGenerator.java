...
public class QtApiGenerator {
	...
	private void printStructureDefinition(PrintWriter out, Struct s) {
		...
		if (s.size == -1) {
			out
					.println(c
							+ s.name
							+ "(const char* _d, quint32 _maxsize) :_data(_d), _size(0)");
			out.println("{");
		} else {
		...
		for (Member m : s.members) {
			int msize = getSize(m);
			if (msize != -1 && s.size == -1) {
				out.println("    if (_position + " + msize
						+ " > _maxsize) return;");
			}
			bytepos = printMemberParser(out, s, m, bytepos);
			out.println("    _position += _msize;");
		}
		if (s.size == -1) {
	...
	private int printMemberParser(PrintWriter out, Struct s, Member m,
			int bytepos) {
		if (m.isArray) {
			if (m.isSimple) {
				printSimpleArrayMemberParser(out, s, m);
			} else {
				out.println("// something is missing for " + m.name
						+ " of type " + m.type().name);
			}
		} else {
			if (m.isSimple) {
				return printSimpleMemberParser(out, s, m, bytepos);
			} else if (m.isStruct) {
				printStructMemberParser(out, s, m);
			} else if (m.isChoice) {
				printChoiceMemberParser(out, s, m);
			} else {
				out.println("// something is missing for " + m.name
						+ " of type " + m.type().name);
			}
	...
	private int printSimpleMemberParser(PrintWriter out, Struct s, Member m,
			int bytepos) {
		out.println("    " + m.name + " = read" + m.type().name
	...
	private void printSimpleArrayMemberParser(PrintWriter out, Struct s,
			Member m) {
		if (m.type().size % 8 != 0) {
			throw new Error(
		...
		out.println("    " + m.name + "Count = " + count + ";");
		out.println("    _msize = " + m.name + "Count*" + (m.type().size / 8)
				+ ";");
		out.println("    " + m.name + " = (const " + type(m)
				+ "*)(_data+_position);");
	}

	private void printStructMemberParser(PrintWriter out, Struct s, Member m) {
		if (m.type().size == -1) {
			out.println("    " + m.name + " = " + m.type().name
					+ "(_data + _position, _maxsize - _position);");
		} else {
			out.println("    " + m.name + " = " + m.type().name
					+ "(_data + _position);");
		}
		if (m.isOptional) {
			out.println("    _msize = (" + m.name + "._data) ?" + m.name
					+ "._size :0;");
		} else {
			out.println("    if (" + m.name + "._data == 0) return;");
			out.println("    _msize = " + m.name + "._size;");
		}
	}

	private void printChoiceMemberParser(PrintWriter out, Struct s, Member m) {
		...
		for (Option o : c.options) {
			...
			if (!first) {
				out.println("    if (_msize == 0) {");
				out.print("    ");
			}
			if (t.size == -1) {
				out.println("    _msize = " + t.name
						+ "(_data + _position, _maxsize - _position)._size;");
			} else {
				out.println("    _msize = (" + t.name
						+ "(_data + _position)._data) ?" + t.name
						+ "::_size :0;");
			}
			if (!first) {
				out.println("    }");
		...
		if (!m.isOptional) {
			out.println("    if (_msize == 0) return;");
		}
	}
